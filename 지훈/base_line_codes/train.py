...
def train_model(LEARNING_RATE=1e-3, EPOCH=5, BATCH_SIZE=64, PRETRAINED=False, LABEL='multi', VAL_SIZE=0.2):
    ...
    for epoch in range(1,EPOCH + 1):
        ...
        if LABEL == 'multi':
                ...
                writer.add_scalar(f"Train/Acc/{label}", acc, epoch)
                
                writer.add_scalar(f"Train/{label}/F1_score", f1_score, epoch)
        ...
        if False:
            ...
            for img, label in val_loader:
                ...
                img, label = img.float().cuda(), torch.tensor(label).cuda()
